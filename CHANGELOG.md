...
## For dbt Spark Users
- The `dbt_pinterest_source` package (which this package has a dependency on) has a dependency on `spark-utils` [">=0.2.0", "<0.3.0"] which does not contain an official full dbt v1 compatibility release. As such, this release does not fully support dbt v1.0.0 for dbt spark users. 
